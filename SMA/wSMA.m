...
function [best_pos,Convergence_curve]=wSMA(N,Max_FEs,lb,ub,dim,fobj)
tic
disp('wSMA is now tackling your problem')
            ...
            for j=1:dim
                ...
                if r<p    %Eq.(2.1)
                    if SmellIndex(i) > 2/N
                        ...
                        X(i,j) = best_pos(j)+ vb(j) * (weight(A, j) * X(A, j)- weight(B, j) * X(B, j));%%
                    end
                else
                    X(i,j) = vc(j)*X(i,j);
                end
